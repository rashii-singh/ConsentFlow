...
import { ShieldCheck, AlertTriangle, CheckCircle2, Hash, FileCheck, Printer, ArrowLeft, Clock, User, Building2 } from 'lucide-react';
...
export default function AuditChainViewer({ record, auditLogs }: AuditChainViewerProps) {
  ...
  const verification = verifyChain(auditLogs);
      ...
      <div className="p-6 sm:p-8 rounded-3xl bg-slate-900 border border-slate-800 space-y-6 shadow-2xl">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-800 pb-4">
          ...
          {verification.valid ? (
            ...
            <div className="inline-flex items-center space-x-2 px-4 py-2 rounded-2xl bg-rose-500/10 border border-rose-500/30 text-rose-400 font-bold text-sm shadow-lg shadow-rose-500/10 self-start sm:self-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-xs font-mono">
          ...
          <div className="p-4 rounded-2xl bg-slate-950 border border-slate-800 space-y-1">
            ...
            <div className="font-bold text-purple-400">{auditLogs.length} Cryptographic Nodes</div>
          </div>
        </div>

        <div className="flex items-center justify-between pt-2">
          <p className="text-xs text-slate-400">
            Every consent grant and revocation recomputes SHA-256(previousHash + canonicalPayload).
          </p>
          ...
          {auditLogs.map((log, idx) => {
            ...
            return (
              <div key={log.id || idx} className="flex items-start space-x-4 relative z-10">
                ...
                <div className="flex-1 p-5 rounded-2xl bg-slate-950 border border-slate-800 space-y-3">
